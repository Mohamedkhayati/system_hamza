...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf']) || $_POST['csrf'] !== $_SESSION['csrf']) {
        ...
        elseif (new DateTime($start_date) > new DateTime()) $error = "Start date cannot be in the future.";
            ...
            $photo = null;
            $allowed_types = ['image/jpeg', 'image/png']; // Define allowed MIME types once
            ...
            if (!$error) {
                ...
                if ($stmt->execute()) {
                    ...
                    header("Location: afficher.php?success=added");
                    exit();
                } else {
...
<head>
    ...
    <title>Add Subscriber - Gym Management System</title>
    <style>
...
<header>
    <nav>
        <ul>
            ...
            <li><a href="ajouter.php" aria-current="page">Add New</a></li>
...
<main>
    ...
    <form method="POST" enctype="multipart/form-data" id="addForm">
        ...
        <label>Number: <input type="number" name="number" required min="1"></label>
        <label>Age: <input type="number" name="age" required min="16" max="100"></label>
        <label>Start Date: <input type="date" name="start_date" id="start_date" required oninput="updateEndDate()"></label>
        <label>Duration:
        ...
        <label>End Date: <input type="date" id="end_date" readonly></label>
        <label>Photo Upload (JPEG/PNG, max 2MB): <input type="file" name="photo" accept="image/jpeg,image/png"></label>
